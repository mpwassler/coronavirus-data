...
def calulate_results_for_day(data, date, previous_day_data):
	...
	return {
		...
		"percent_recovered"     : round((recovered / cases) * 100, 4),
	}
...
def print_results_table(data):
	...
	output_headers = ["Date", "Cases", "Cases % ^", "Deaths", "Deaths % ^", "Recovered", "% cases recovered", "Case Fatality Rate" ]
...
def draw_growth_plot(data):
	...
	percent_recovered =    	[ row["percent_recovered"]     for row in data ]
	...
	plt.plot( range(len(data)), percent_recovered, 'g:',  label='% Cases recovered' )
	plt.legend()
